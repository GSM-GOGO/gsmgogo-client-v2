...
import { useRankData, useMyId } from '../../apis/Ranking/ranking'
...
const Content = () => {
  const { data: RankData, isLoading: isRankLoading } = useRankData()
  const { data: myId, isLoading: isMyIdLoading } = useMyId()
  ...
  const topThreeDatatest =
    RankData.length > 0
      ? RankData.sort((a, b) => b.user_point - a.user_point).slice(0, 3)
      : []

  const firstPlacePointstest =
    topThreeDatatest.length > 0 ? topThreeDatatest[0].user_point : 0
  const secondPlacePointstest =
    topThreeDatatest.length > 1 ? topThreeDatatest[1].user_point : 0
  const thirdPlacePointstest =
    topThreeDatatest.length > 2 ? topThreeDatatest[2].user_point : 0

  const secondPlaceHeighttest =
    (260 / firstPlacePointstest) * secondPlacePointstest
  const thirdPlaceHeighttest =
    (260 / firstPlacePointstest) * thirdPlacePointstest
    ...
    <>
      <S.MainContainer>
          ...
          {RankData.sort((a, b) => b.user_point - a.user_point)
            ...
            .map((_, index) => {
              const {
                ...
                rankComponent,
                rank,
              } = getRankInfotest(index, RankData)
              return (
        ...
        <S.ListWrapper>
                ...
                <S.List myrank={item.user_id === myId} key={index}>
                  <S.TextContainer>
                    <S.Text>
                      ...
                      <S.Name>
                        {Number[item.user_grade]}-{Number[item.user_class]}{' '}
                        {item.user_name}
                      </S.Name>
                    </S.Text>
                  </S.TextContainer>
          ...
          <S.Stroke />
          {RankData.sort((a, b) => b.user_point - a.user_point).map(
            (item, index) => (
              <S.List myrank={item.user_id == myId} key={index}>
                <S.TextContainer>
                  <S.Text>
